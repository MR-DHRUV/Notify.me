...
const ReminderState = (props) => {
  ...
  const getAllReminders = async () => {
    const response = await fetch('https://api-data-notify.herokuapp.com/reminder/fetch_all_reminders', {
      method: 'GET',
      headers: {
  ...
  const addReminder = async (title, description,time) => {
    ...
    const response = await fetch('https://api-data-notify.herokuapp.com/reminder/add_reminder', {
  ...
  const deleteReminder = async (id) => {
    ...
    //eslint-disable-next-line
    const response = await fetch(`https://api-data-notify.herokuapp.com/reminder/delete_reminder/${id}`, {
      method: 'GET',
      headers: {
        'Content-Type': 'application/json',
        'auth-token': localStorage.getItem('token')
  ...
  const editReminder = async (id, title, description, time) => {
    //API Call
    //eslint-disable-next-line
    const response = await fetch(`https://api-data-notify.herokuapp.com/reminder/update_existing_reminder/${id}`, {
