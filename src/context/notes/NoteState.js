...
const NoteState = (props) => {
  ...
  const getAllNotes = async () => {
    const response = await fetch('https://api-authify.herokuapp.com/notes/fetch_all_notes', {
      method: 'GET',
      headers: {
  ...
  const addNote = async (title, description, tag) => {
    ...
    const response = await fetch('https://api-authify.herokuapp.com/notes/add_note', {
  ...
  const editNote = async (id, title, description, tag) => {
    //API Call
    //eslint-disable-next-line
    const response = await fetch(`https://api-authify.herokuapp.com/notes/update_existing_note/${id}`, {
